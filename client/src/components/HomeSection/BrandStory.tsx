...
const BRAND_DATA = {
  ...
  description:
    "From 200ml event bottles ASVA offers safe alkaline hydration formats for every use, home dining, clinics, gym , office pantries and hotel guests.",
  image: brandWoman,
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center justify-center max-w-[1280px] mx-auto">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center text-left lg:pl-10">
          <div className="animate-in fade-in slide-in-from-right-8 duration-1000">
            <span className="text-label mb-4 block">{BRAND_DATA.tag}</span>

            <h2 className="text-h1 mb-6">{BRAND_DATA.title}</h2>

            <p className="mb-8 max-w-[540px]">{BRAND_DATA.description}</p>

            <div className="space-y-4 mb-12">
              {/* <ul className="space-y-3">
                                {[BRAND_DATA.listOne, BRAND_DATA.listTwo].map((item, i) => (
                                    <li key={i} className="flex items-center gap-3 text-[#393939]/90 text-[15px] md:text-[14px]">
            ...
            <Link
              to={BRAND_DATA.link}
              className="group relative inline-flex items-center gap-4 text-[14px] font-bold text-black uppercase tracking-widest transition-all"
            >
