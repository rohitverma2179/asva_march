    ...
    <div className="flex flex-col group">
      ...
      <div className="relative aspect-square bg-white rounded-lg mb-2 md:mb-6 flex items-center justify-center p-2 md:p-8">
        ...
        <div className="absolute top-3 right-3 md:top-5 md:right-5 flex flex-col gap-2 z-10">
          {product.badges.map((badge, index) => (
            <div
              key={index}
              className="w-9 h-14 md:w-14 md:h-24 bg-sky-200/60 backdrop-blur-md rounded flex items-center justify-center text-[11px] md:text-[18px] font-bold text-black border border-sky-100 shadow-sm"
            >
              <div className="flex flex-col items-center justify-center h-full">
                {badge.toLowerCase().includes("ml") ? (
                  <div className="flex flex-col items-center">
                    <span className="leading-none tracking-tighter">
                      {badge.toLowerCase().replace("ml", "").trim()}
                    </span>
                    <span className="text-[8px] md:text-[10px] uppercase font-extrabold mt-1 tracking-widest opacity-80">
                      ml
                    </span>
                  </div>
                ) : (
                  <span className="whitespace-nowrap leading-none px-1 text-center">
                    {badge}
                  </span>
                )}
              </div>
      ...
      <div className="flex flex-col gap-1 md:gap-2 ml-1 md:ml-7 items-start">
        ...
        {product.price.includes(" | ") ? (
          <div className="flex items-center gap-1 md:gap-2 text-sm md:text-[14px] text-gray-500 font-medium">
            <s className="opacity-60 text-[10px] md:text-[13px]">
        ...
        <Link
          ...
          className="mt-1 md:mt-2 text-xs md:text-[14px] font-bold text-black border-b border-white pb-0.5 hover:border-black uppercase tracking-widest transition-colors"
        >
          Shop
        </Link>
      </div>
